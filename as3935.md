...
_data_binding_ - Specify a data binding to save lightning data to a separate lightning database.  For example, `data_binding = lightning_sqlite` will save data to a sqlite database in the same directory as the other weewx databases.  The lightning database schema is a format that works with the weewx reporting and plotting utilities, so you can extract lightning data and use it in standard weewx reports and plots.
