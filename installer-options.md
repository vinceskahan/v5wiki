...
A pip install pulls software from the pypi repository, or a weewx-hosted repository.  A simple example looks like this:
```
pip install weewx
```
...
+ Does not necessarily require root privileges to modify config files or skins.
...
+ works well with virtual environments. Can even be automated using `pipenv`.
...
- no mechanism for udev scripts (is there one for package installers? -tk).<br/>
...
- entry points.  Pip installs entry points in a separate `bin` directory within whichever python the pip is associated. Modern
  ...
  entry points. (Why is this a con? -tk)
- where do the config files and skins live?<br/>
  The config and skins are part of the python code, so they go into the Python tree, under the weewx package. (Why is this a con? -tk)
...
- no standard for ancillary configs such as logrotate, rsyslog, udev, nginx (Is there for package installers? In any case, these are outside the domain of a WeeWX install. -tk)
- no mechanism for installing multiple concurrent versions<br/>
  This can be done with separate virtual environments. (Not sure what you mean by "concurrent versions"? If you mean multiple instances of weewx running, they can all share the same code base and skins. If you mean multiple code bases then, yes, they would each require a separate vm. But, how would you do this for package installers? Why is it not a 'con' for them? -tk)
- you must install weewx for each python environment.<br/>
  Since weewx is installed into the python directory tree, you cannot use a single weewx tree with multiple python versions.  As a result, this installation method is not ideal for the development use case. (I do this all the time with `pyenv` or `venv`: one code base, multiple versions of Python. -tk)
...
- requires root to install and update.
- Requires root to change config files or skins.
- Bits and pieces of WeeWX are spread all over the file system. 
- Not possible to run multiple WeeWX code bases.
...
- No isolation: updates run the risk of breaking other system code.
