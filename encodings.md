...
There are four places where encodings come into play in WeeWX:
...
that if your program has a line like this
...
tells the WeeWX to use HTML entities to encode any non-ASCII characters. If the current
temperature is 20C, then the tag `$current.outTemp` will result in either
...
If, instead, you were to specify UTF-8:
...
that template uses. This is why almost all WeeWX templates start with the following:
...
UTF-8. Which brings us to the final kind of encoding directive.
...
interpret any HTML tags, including a `<meta>` tag, so it does not know what encoding the file uses.
It has to guess.
...
default encoding for NOAA files.
...
can be represented in `cp1252`.

What will not work is an encoding of UTF-8. For example, the degree symbol, which takes two bytes
to encode using UTF-8, is instead interpreted by the browser as two characters encoded in cp1252,
in this case, the character 'Á' and the character '⁰'.
